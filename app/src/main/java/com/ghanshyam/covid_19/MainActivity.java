...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    com.ghanshyam.covid_19.Adapter adapter;
    ...
    private void fetchdata() {
        ...
        ApiUtilities.getAPIInterface().getcountrydata().enqueue(new Callback<List<ModelClass>>() {
            @Override
            public void onResponse(Call<List<ModelClass>> call, Response<List<ModelClass>> response) {
                modelClassesList.addAll(response.body());
                for (int i = 0; i < modelClassesList.size(); i++) {
                    if (modelClassesList.get(i).getCountry().equals(country)) {
                        mactive.setText((modelClassesList.get(i).getActive()));
                        mtodaydeaths.setText((modelClassesList.get(i).getTodayDeaths()));
                        mtodayrecovered.setText((modelClassesList.get(i).getTodayRecovered()));
                        mtodaytotal.setText((modelClassesList.get(i).getTodayCases()));
                        mtotal.setText((modelClassesList.get(i).getCases()));
                        mdeaths.setText((modelClassesList.get(i).getDeaths()));
                        mrecovered.setText((modelClassesList.get(i).getRecovered()));

                        int active, total, recovered, deaths;

                        active = Integer.parseInt(modelClassesList.get(i).getActive());
                        total = Integer.parseInt(modelClassesList.get(i).getCases());
                        recovered = Integer.parseInt(modelClassesList.get(i).getRecovered());
                        deaths = Integer.parseInt(modelClassesList.get(i).getDeaths());
